...
namespace UnityEngine.PostProcessing
{
    ...
    public sealed class EyeAdaptationComponent : PostProcessingComponentRenderTexture<EyeAdaptationModel>
    {
        ...
        public Texture Prepare(RenderTexture source, Material uberMaterial)
        {
            ...
            {
                var gazePos = TobiiAPI.GetGazePoint().Viewport;

                gazePos = new Vector2(Mathf.Clamp(gazePos.x, 0, 1), Mathf.Clamp(gazePos.y, 0, 1));
               
                // Send gaze relative position to eye histogram computed shader
                m_EyeCompute.SetVector("_GazeViewportPosition", gazePos);
            }
            else
            {
                var viewportPositionOfScreenCenter = new Vector2(0.5f, 0.5f);
                m_EyeCompute.SetVector("_GazeViewportPosition", viewportPositionOfScreenCenter);
            }
