...
public class FingerPinch : MonoBehaviour 
{
    ...
    [SerializeField] bool m_IsEnteredPinch;//是否进入了捏动作的匹配状态，即是否检测到捏的初始状态，并且到现在为止动作也是正常匹配中。
    readonly float EnterDisThreshold=80f;//>80mm判定进入Pinch状态
    ...
    //Pinched
    readonly float PinchedDisThreshold=10f;//Leap单位，10mm
    Action m_OnPinched;//完成捏动作匹配后，触发事件链。
